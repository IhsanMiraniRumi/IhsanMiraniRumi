# 👨‍💻 Ihsan Mirani Rumi — Developer Portfolio

Welcome to my personal portfolio website, crafted with minimalist design using **HTML** and **Tailwind CSS**, and hosted via **GitHub Pages**.

🔗 **Live Site:** [ihsanmirani.github.io](https://ihsanmirani.github.io)  
📄 **Online CV:** [ihsanmiranirumi.vercel.app](https://ihsanmiranirumi.vercel.app)  
📧 **Email:** ihsanmirani9865@gmail.com

---

## 🧠 About Me

I’m a Graduate in Computer Science and Engineering from **Shahjalal University of Science and Technology (SUST)** with a strong passion for:

- 🔍 Full-stack web development  
- 🤖 AI & machine learning applications  
- 🚀 DevOps tools and modern deployment  
- ⚙️ Problem solving and competitive programming  

I bring a blend of **software development**, **AI/ML skills**, and **research mindset**, backed by hands-on projects and real-world challenges.

---

## 🛠️ Tech Stack

| Area                     | Technologies & Tools |
|--------------------------|----------------------|
| **Languages**            | Python, JavaScript, C++, Java |
| **Web & Frameworks**     | Django, React.js, Node.js, Express.js, Streamlit |
| **Databases**            | MySQL, PostgreSQL, MongoDB |
| **Machine Learning**     | PyTorch, TensorFlow, Keras, Scikit-learn |
| **DevOps**               | Docker, GitHub, AWS Cloud |
| **UI/Styling**           | HTML, CSS, Tailwind CSS, SASS |
| **Other Tools**          | Git, LaTeX, Postman, REST APIs |
| **Problem Solving**      | 500+ solved (LeetCode, Codeforces, Vjudge) |

---

## 🚀 Highlight Projects

### 📝 Blog Application  
**Django + REST API + PostgreSQL**  
🔗 [GitHub Repository](https://github.com/IhsanMiraniRumi/blog-application)  
- CRUD-based full-stack blog platform with RESTful backend  
- SEO optimized with structured metadata  
- Designed for scalability, clean URLs, and responsiveness  

---

### 💊 MediLuxe — Pharmacy Management System  
**React.js + Node.js + MongoDB + Redux**  
🔗 [GitHub Repository](https://github.com/IhsanMiraniRumi/pharmacy-management-system)  
- Role-based system with secure JWT login and session control  
- Advanced filtering, search, and inventory management  
- Real-time order tracking and delivery flow

---

## 📊 Research & Thesis

### 📘 Comparative Analysis of Emotional Expressions: KDEF vs Bengali Dataset  
**Supervisor:** Dr. Sadia Sultana, Associate Professor  
- Created the **first culturally-specific Bengali facial expression dataset**  
- Used **OpenFace** and **FACS** to extract 16 AUs  
- Trained **ResNet-18** and **Vision Transformer** for cross-cultural emotion detection  
- Tackled bias in AI by comparing emotion recognition on Western vs. Bengali expressions

---


## 🎓 Education

- **B.Sc. in CSE**, Shahjalal University of Science and Technology (SUST)  
  _2020–2025_ | **CGPA:** 3.45/4.00  
- **HSC**, Notre Dame College  
  _2017–2019_ | **GPA:** 5.0 (with General Scholarship)  
- **SSC**, Pabna Zilla School  
  _2015–2017_ | **GPA:** 5.0 (with General Scholarship)

---

## 🏆 Achievements & Recognition

- 🥇 **SUST CSE Hackathon 2024**  
  Finalist from 100+ teams — Built a full-stack e-commerce app integrated with AI  
  ![Certificate](https://github.com/IhsanMiraniRumi/certificates/blob/main/sust-cse-hackathon-24.png)

- 🧮 **ICPC Asia Dhaka Regional 2021**  
  Ranked 267 out of 1272 teams (Team: SUST_IronClad)  
  ![Placement](https://github.com/IhsanMiraniRumi/certificates/blob/main/2021-Asia%20Dhaka%20Preliminary-Ihsan%20Mirani%20Rumi-PLACE.jpg)

---

## 👥 Leadership Experience

### General Secretary  
**Pabna Students’ Association, SUST (2024–Present)**  
- Led initiatives for 100+ students from Pabna  
- Organized cultural events, tours, and a successful winter BBQ  
- Coordinated with juniors, seniors, and university officials  
🔗 [News Link](https://www.natunsylhet24.com/details/21696)

---



## 📌 Interests

- Artificial Intelligence & Affective Computing  
- Web Application Design  
- Open Source Contributions  
- Traveling, Video Games, Football  

---

## 📬 Let's Connect

Feel free to reach out if you'd like to collaborate on projects, research, or job opportunities!

- 📧 **Email:** ihsanmirani9865@gmail.com  
- 🌐 [Portfolio Website](https://ihsanmirani.github.io)  
- 💼 [LinkedIn](https://www.linkedin.com/in/ihsanmiranirumi/)  
- 🛠️ [GitHub](https://github.com/IhsanMiraniRumi)

---

> “I create smooth digital experiences by combining software development with AI and machine learning, building smart, user-friendly solutions that connect creativity and technology.” — *Ihsan Mirani Rumi*
